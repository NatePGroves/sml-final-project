...
```
```{r}
# Function to print dimensions after removing NAs
print_clean_dims <- function(df, name) {
  clean_df <- na.omit(df)
  cat(name, ":", dim(clean_df)[1], "rows,", dim(clean_df)[2], "columns\n")
}

# Print dimensions for each
print_clean_dims(circulation, "circulation")
print_clean_dims(demographics, "demographics")
print_clean_dims(electronic, "electronic")
print_clean_dims(programs, "programs")
print_clean_dims(revenue, "revenue")
print_clean_dims(services, "services")
```
